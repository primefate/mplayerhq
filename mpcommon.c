...
static int get_win32_cmdline(int *argc_ptr, char **argv_ptr[])
{
    ...
    argv_w = CommandLineToArgvW(GetCommandLineW(), &argc_n);
    if (!argv_w || argc_n < 0 || argc_n >= INT_MAX / sizeof(char *))
        goto err_out;

    size = argv_size = (argc_n + 1) * sizeof(char *);
    for (i = 0; i < argc_n; i++) {
        int conv_size = WideCharToMultiByte(CP_UTF8, 0, argv_w[i], -1, NULL, 0, NULL, NULL);
    ...
    for (i = 0; i < argc_n; i++) {
        int conv_size = WideCharToMultiByte(CP_UTF8, 0, argv_w[i], -1,
